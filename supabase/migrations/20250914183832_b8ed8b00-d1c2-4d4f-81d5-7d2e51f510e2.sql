-- Secure phone change system with OTP verification - Final fix
...
BEGIN
  IF NOT EXISTS (SELECT 1 FROM information_schema.table_constraints 
                 WHERE constraint_name = 'users_app_auth_uid_unique' 
                 AND table_name = 'users_app') THEN
    ALTER TABLE users_app ADD CONSTRAINT users_app_auth_uid_unique UNIQUE (auth_user_id);
  END IF;
END$$;
...
-- Create function to check if OTP is active (immutable)
CREATE OR REPLACE FUNCTION is_otp_active(expires_at timestamptz)
RETURNS boolean
LANGUAGE sql
IMMUTABLE
AS $$
...
-- Only one active OTP per (user, phone) at a time - using immutable function
CREATE UNIQUE INDEX IF NOT EXISTS otp_codes_active_unique
  ON otp_codes (user_auth_id, new_phone_e164)
  WHERE is_otp_active(expires_at);
