...
-- Create message log table (inbound + outbound)
...
create table if not exists daily_prompts (
  ...
  sent_at timestamptz default now(),
  sent_date date generated always as (date_trunc('day', sent_at)::date) stored
);

-- Create unique constraint for daily prompts (one prompt per user per day)
create unique index if not exists daily_prompts_user_date_idx 
...
-- Create RLS policies
-- Users can read their own entries (for future frontend)
create policy "read_own_entries_future_frontend"
on journal_entries for select
to authenticated
using (auth.uid() is not null);

-- Service role has full access (implicit via service key)
