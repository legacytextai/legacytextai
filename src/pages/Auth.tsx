        ...
        <div className="w-full max-w-md">
          ...
          <Card className="border-legacy-border bg-card/80 backdrop-blur-sm">
            <CardHeader className="text-center">
              <CardTitle className="text-2xl font-bold text-legacy-primary">Welcome to LegacyText</CardTitle>
              <CardDescription className="text-legacy-ink/70">
                Create your account or sign in to start building your legacy journal
              </CardDescription>
            </CardHeader>
            <CardContent>
              {emailConfirmed && <Alert className="mb-4 border-green-500/50 bg-green-50/50">
                    ...
                    <div className="space-y-2">
                     ...
                     <div className="space-y-2">
                       <Label htmlFor="signup-phone">Phone Number</Label>
                        <Input id="signup-phone" type="tel" placeholder="(123) 456-7890" value={phoneDisplay} onChange={e => {
                      const value = e.target.value;
                      setPhoneDisplay(formatPhoneDisplay(value));
                      setPhone(normalizePhoneToE164(value));
                    }} required className={phoneDisplay && !isValidPhone(phoneDisplay) ? "border-destructive" : ""} />
                        {phoneDisplay && !isValidPhone(phoneDisplay) && <p className="text-sm text-destructive">
                            Please enter a valid US phone number (10 digits)
                          </p>}
                     </div>
                      <Button type="submit" disabled={loading || !isValidPhone(phoneDisplay)} className="w-full">
                        <Mail className="w-4 h-4 mr-2" />
              ...
              {showResend && <div className="mt-4 p-3 bg-legacy-ink/5 rounded-lg border border-legacy-border">
                  <p className="text-sm text-legacy-ink/70 mb-2">
                    Didn't receive the confirmation email?
                  </p>
                  <Button onClick={handleResendConfirmation} disabled={resendLoading} variant="outline" size="sm" className="w-full">
